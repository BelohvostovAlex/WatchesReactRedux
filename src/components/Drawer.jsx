  ...
  const onClickOrder = async () => {
    try {
      ...
      let response = await axios.post(
        "https://6192739c57b14a0017c4a0c6.mockapi.io/order",
        { items: cartItems }
      );
      for (let i = 0; i < cartItems.length; i++) {
        ...
        await delay(
          100,
          axios.delete(
            `https://6192739c57b14a0017c4a0c6.mockapi.io/carts/${item.id}`
          )
        );
      }
  ...
  return (
    <div className={`overlay ${opened ? "overlayVisible" : ""} `}>
      <div className="drawer">
        <h2>
          Cart
          <button className="button button-remove" onClick={onClose}>
            <img
              className="removeImg"
              width={20}
              height={20}
        ...
        {cartItems.length > 0 ? (
          ...
          <Info
            title={
              isOrderDone
                ? `Your order #${orderId} is completed`
                : "Cart is empty"
            }
            description={
              isOrderDone
                ? "Soon ur super mega watches will be on ur hand"
                : "Please add a watch that u are want and deserve!"
            }
