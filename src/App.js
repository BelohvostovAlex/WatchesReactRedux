...
function App() {
  ...
  React.useEffect(() => {
    async function fetchData() {
      try {
        setIsLoading(true);
        const [watchesResponse, cartItemsResponse, likedItemsResponse] =
          await Promise.all([
  ...
  const onAddToCart = async (obj) => {
    try {
      ...
      if (findItem) {
        setCartItems((prev) =>
          prev.filter((item) => item.itemId !== obj.itemId)
        );
        await axios.delete(
          `https://6192739c57b14a0017c4a0c6.mockapi.io/carts/${findItem.id}`
        );
      } else {
        let response = await axios.post(
          "https://6192739c57b14a0017c4a0c6.mockapi.io/carts",
          obj
        );
  ...
  const onLikeItems = async (obj) => {
    try {
      if (
        ...
        axios.delete(
          `https://6192739c57b14a0017c4a0c6.mockapi.io/liked/${obj.id}`
        );
      } else {
        const { data } = await axios.post(
          "https://6192739c57b14a0017c4a0c6.mockapi.io/liked",
          obj
        );
  ...
  return (
    <AppContext.Provider
      value={{ watches, cartItems, setCartItems, likedItems, isItemAdded }}
    >
      <div className="wrapper">
        ...
        <Routes>
          ...
          <Route
            path="/favourites"
            element={<Favourites onLikeItems={onLikeItems} />}
            exact
          ></Route>
